...
describe('pinnedJobsStore', () => {
  ...
  it('should pin jobs and handle max size correctly', () => {
    ...
    const jobs = Array.from({ length: 505 }, (_, i) => ({ id: i, name: `job-${i}` }));
    ...
    expect(notify).toHaveBeenCalledWith('Max pinboard size of 500 reached.', 'danger', { sticky: true });
  });

  it('should not show error if all jobs to pin are already pinned', () => {
    const store = usePinnedJobsStore.getState();
    const jobs = Array.from({ length: 10 }, (_, i) => ({ id: i, name: `job-${i}` }));

    store.pinJobs(jobs);
    jest.clearAllMocks();

    store.pinJobs(jobs);
    expect(Object.keys(usePinnedJobsStore.getState().pinnedJobs).length).toBe(10);
  ...
  it('should only pin what fits and show error if some dont fit', () => {
    const store = usePinnedJobsStore.getState();
    // Fill up to 495
    const existingJobs = Array.from({ length: 495 }, (_, i) => ({ id: i, name: `job-${i}` }));
    store.pinJobs(existingJobs);

    // Try to add 10 more
    const moreJobs = Array.from({ length: 10 }, (_, i) => ({ id: i + 500, name: `job-${i + 500}` }));
